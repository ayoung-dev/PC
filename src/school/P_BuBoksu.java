...
public class P_BuBoksu extends JFrame {
	...
	public P_BuBoksu(String id) throws Exception {
		...
		//휴학 신청 확인
        String colNames[] = {"학번", "이름", "학년도", "학기","부/복수 구분", "전공명", "신청일", "처리 상태"};
        String rows[][] = {};
        ...
        JLabel Title = new JLabel("부/복수 신청 조회");
     	...
     	Delete.addActionListener(new ActionListener() {
  			public void actionPerformed(ActionEvent e) {
  				try {
					...
					pm.deleteMinDouble(stNum, now);
					model.removeRow(row);
					JOptionPane.showMessageDialog(null,"해당 부/복수 전공 신청이 취소되었습니다.");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
     	...
     	Confirm.addActionListener(new ActionListener() {
  			public void actionPerformed(ActionEvent e) {
  				try {
  					...
  					String stNum = (String) table.getValueAt(row, 0);
  					String now = (String) table.getValueAt(row, 6);
					pm.acceptMinDouble(stNum, now);
					JOptionPane.showMessageDialog(null,"해당 부/복수 전공 신청이 승인되었습니다.");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
        ...
        // 해당 학생 신청 여부 띄우기
     	...
     	if (rs.getRow() > -1) {
     		while (rs.next()) {
     			...
     			// GUI에 띄우기
     			String[] row = { num, name, ma.getApSemester().substring(0,4), ma.getApSemester().substring(5), ma.getClassify(),
     					ma.getApMajor(), ma.getApDate(), ma.getApState()};
