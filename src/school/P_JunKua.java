...
public class P_JunKua extends JFrame {
	...
	public P_JunKua(String id) throws Exception {
		...
		//휴학 신청 확인
        String colNames[] = {"학번", "이름", "학년도", "학기", "신청 전공", "성적", "신청 일자", "처리 상태"};
        String rows[][] = {};
        ...
        JLabel Title = new JLabel("전과 신청 조회");
        Title.setFont(new Font("한컴산뜻돋움", Font.BOLD, 23));
        Title.setBounds(12, 210, 198, 27);
        Main.add(Title);
     	...
     	Delete.addActionListener(new ActionListener() {
  			public void actionPerformed(ActionEvent e) {
  				try {
  					...
  					String now = (String) table.getValueAt(row, 6);
					pm.deleteChange(stNum, now);
					model.removeRow(row);
					JOptionPane.showMessageDialog(null,"해당 전과 신청이 취소되었습니다.");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
     	...
     	Confirm.addActionListener(new ActionListener() {
  			public void actionPerformed(ActionEvent e) {
  				try {
  					...
  					String stNum = (String) table.getValueAt(row, 0);
  					String now = (String) table.getValueAt(row, 6);
					pm.acceptChange(stNum, now);
					JOptionPane.showMessageDialog(null,"해당 전과 신청이 승인되었습니다.");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
        ...
        // 해당 학생 신청 여부 띄우기
     	...
     	if (rs.getRow() > -1) {
     		while (rs.next()) {
     			...
     			MajorApplication ma = sm.getMa(num, now, what);
     			// GUI에 띄우기
     			String[] row = { num, name, ma.getApSemester().substring(0,4), ma.getApSemester().substring(5), ma.getApMajor(), 
     					Float.toString(sm.getTotalGrade(num, "총점")), ma.getApDate(), ma.getApState()};
