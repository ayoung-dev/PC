...
public class S_Bokhak extends JFrame {
	...
	public S_Bokhak(String id) throws Exception {
        ...
        JLabel Submit_Year = new JLabel("복학 연도 : " + hok_year);
        Submit_Year.setBounds(12, 11, 120, 21);
        panel_1.add(Submit_Year);
        
        JLabel Submit_Semester = new JLabel("복학 학기 : " + sem);
        Submit_Semester.setBounds(204, 11, 77, 21);
        panel_1.add(Submit_Semester);
        
        
        JLabel Submit_Date = new JLabel("신청 날짜 : " + strDate);
        	...
        	public void actionPerformed(ActionEvent e) {
  				try {
  					sm.addGoBack(sm.getstName(id), id, hok_year + "-" + sem, "", "", 
								"", 0, allDate);
  					JOptionPane.showMessageDialog(null,"복학신청이 완료되었습니다.");
				} catch (Exception e1) {
					e1.printStackTrace();
				}
        	}
		...
		//휴학 정보 띄우기        
        String colNames[] = {"휴학 사유", "휴학 신청 학년도", "휴학 학기 수", "휴학 시작 일자", "휴학 종료 일자"
        		, "복학 예정 학년도", "복학 예정 학기"};
        String rows[][] = {};
        ...
        SystemMainName.setFont(new Font("한컴산뜻돋움", Font.BOLD, 20));
        SystemMainName.setText("복학신청");
