...
public class P_Huhak extends JFrame {
	...
	public P_Huhak(String id) throws Exception {
		...
		//휴학 신청 확인
        String colNames[] = {"학번", "이름","학년도", "학기", "시작 일자", "종료 일자", "변동 사유",
        		"휴학 학기 수", "신청일", "처리 상태"};
        String rows[][] = {};
        ...
        JLabel ProfHuhak = new JLabel("휴학 신청 조회");
        ProfHuhak.setFont(new Font("한컴산뜻돋움", Font.BOLD, 23));
        ProfHuhak.setBounds(12, 210, 172, 27);
        Main.add(ProfHuhak);
     	...
     	Delete.addActionListener(new ActionListener() {
  			public void actionPerformed(ActionEvent e) {
  				try {
  					...
  					String now = (String) table.getValueAt(row, 8);
					pm.deleteLeave(stNum, now);
					model.removeRow(row);
					JOptionPane.showMessageDialog(null,"해당 휴학 신청이 취소되었습니다.");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
     	...
     	Confirm.addActionListener(new ActionListener() {
  			public void actionPerformed(ActionEvent e) {
  				try {
  					...
  					String stNum = (String) table.getValueAt(row, 0);
  					String now = (String) table.getValueAt(row, 8);
					pm.acceptLeave(stNum, now);
					JOptionPane.showMessageDialog(null,"해당 휴학 신청이 승인되었습니다.");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
        ...
        // 해당 학생 신청 여부 띄우기
     	...
     	if (rs.getRow() > -1) {
     		while (rs.next()) {
     			...
     			// GUI에 띄우기
     			String[] row = { num, name, sa.getApSemester().substring(0,4), sa.getApSemester().substring(5), sa.getStartDate(), sa.getEndDate(),
